...
class ChallengeTest < ActiveSupport::TestCase
  test 'normal challenge must have 2 users' do
    ...
    assert challenge.errors.full_messages.include? 'Users only two users are allowed in a normal challenge'
    challenge.users << build(:user)
    challenge.valid?
    refute challenge.errors.full_messages.include? 'Users only two users are allowed in a normal challenge'
  end
  ...
  test 'open_spot challenge must have 2 users' do
    ...
    challenge.users << build(:user)
    challenge.valid?
    refute challenge.errors.full_messages.include? 'Users no more than two users are allowed in an open spot challenge'
    challenge.users << build(:user)
    challenge.valid?
    refute challenge.errors.full_messages.include? 'Users no more than two users are allowed in an open spot challenge'
    challenge.users << build(:user)
    refute challenge.valid?
    assert challenge.errors.full_messages.include? 'Users no more than two users are allowed in an open spot challenge'
  end
  ...
  test 'tri challenge must have 3 users' do
    ...
    challenge.users << build(:user)
    refute challenge.valid?
    assert challenge.errors.full_messages.include? 'Users only three users are allowed in a tri challenge'
    challenge.users << build(:user)
    challenge.valid?
    refute challenge.errors.full_messages.include? 'Users only three users are allowed in a tri challenge'
  end
